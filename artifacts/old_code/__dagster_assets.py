...
OUTPUT_PATH = file_relative_path(__file__, "../cian_dataset.dill")
LOAD_PATH = file_relative_path(__file__, "../cian_dataset_enriched.dill")

# Фикс 429-й от циана
CONCURRENT_REQUESTS = 5

async def fetch_page(session, url, max_retries=5):
    attempt = 0
        ...
        try:
            async with session.get(url, headers=local_headers) as response:
                if response.status == 429:
                    ...
                    print(f"Получен {response.status} для {url}. Меняем User-Agent и повторяем запрос.")
...
def scrape_it():
    ...
    loaded_flats = load_existing_data()
...
daily_refresh_job = dg.define_asset_job(
    "daily_refresh", selection=["scrape_it", "enricher"]
)
